...
if uploaded_file is not None:
    ...
    if pd.api.types.is_numeric_dtype(data[column_select]):
        ...
        try:
            data[column_select] = pd.to_datetime(
                data[column_select], errors='coerce', infer_datetime_format=True
            )
        except Exception as e:
            st.warning(f"Could not convert column to datetime: {e}")
    ...
    st.info("Please upload your train.csv file to get started.")
